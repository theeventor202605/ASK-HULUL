...
function createProject(user, p) {
  requirePermission(user, 'project.manage'); // RBAC pilot -- same default roles as before, no behavior change
...
function deleteProject(user, p) {
  var project = getById('Projects', p.projectId);
  if (!project) throw new HululError('NOT_FOUND', 'Project not found');
  requirePermission(user, 'project.delete'); // RBAC pilot -- same default roles as before, no behavior change
