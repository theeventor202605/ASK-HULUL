...
async function renderUsers() {
  ...
  if (creatable.length) document.getElementById('newUserBtn').onclick = () => openNewUserModal(creatable);
...
function openNewUserModal(creatableRoles) {
  var body =
    ...
    (HululState.user.role === 'SystemAdmin' ? UI.field('Org ID', '<input id="fUOrg" class="field-input" placeholder="ORG-0001" />') : '');
  UI.openModal(t('new_event') === 'New Event' ? 'New account' : 'New account', body, [
    ...
    { label: t('create'), className: 'btn-primary', onClick: async function () {
        try {
          ...
          var orgInput = document.getElementById('fUOrg');
          if (orgInput) payload.orgId = orgInput.value;
