...
2. Name it (e.g. "HULUL login"). Under **Supported account types**, pick the option matching who
   should be able to sign in (usually "Accounts in this organizational directory only" for a
...
6. In HULUL: **Settings → Single Sign-On → Microsoft sign-in** — check **Enable Microsoft
   sign-in**, paste the Application (client) ID and Directory (tenant) ID, click **Save**.
7. Log out and confirm a "Sign in with Microsoft" button now appears on the login screen, and that
   it successfully signs you into your own (Active, email-matching) HULUL account. The first
   sign-in for any given Microsoft account may prompt an admin/user consent screen — that's Entra
   ID itself, not a HULUL bug.
